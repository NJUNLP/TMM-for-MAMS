...
from instance import ATSAInstance
import torch.nn.functional as F
from module_ATSA import RobertaATSA



def train(epoches,configs,model_name):
    model=RobertaATSA(configs).to(configs.DEVICE)

    instances_train = pickle.load(open('data/ATSA/train.pickle','rb'))
    instances_dev = pickle.load(open('data/ATSA/dev.pickle', 'rb'))

    instances_train=utils.merge_instances(instances_train)
    instances_dev=utils.merge_instances(instances_dev)
    ...
    for i in range(epoches):
        for j in tqdm(range(trainset.batch_count)):
            ...
            predictions=model.forward_ATSA(batch)
        ...
        print('epoch:{},ATSA-accuracy:{},precision:{},recall:{},f1:{}'.format(i,acc, precision, recall, f1))
...
def test(model_name,configs):
    instances_test=pickle.load(open('data/ATSA/test.pickle', 'rb'))
    instances_test=utils.merge_instances(instances_test)

    testset=Dataset(instances_test,configs.BATCH_SIZE,shuffle=False)
    model=RobertaATSA(configs).to(configs.DEVICE)
    model.load_state_dict(torch.load(model_name))
    acc, precision, recall, f1 = evaluate(model, testset)
    print('ATSA-accuracy:{:.5f},precision:{:.5f},recall:{:.5f},f1:{:.5f}'.format(acc, precision, recall, f1))

if __name__=='__main__':
    configs=RobertaConfigs()
    model_name='roberta_atsa'

    train(15,configs,model_name)
